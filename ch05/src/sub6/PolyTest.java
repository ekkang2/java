 ...
 *  - 객체(참조변수 a1, a2, a3)의 타입선언을 부모클래스 타입으로 선언(업캐스팅)
...
public class PolyTest {
	...
	public static void main(String[] args) {
		...
		// 다형성 적용된 객체 생성(업캐스팅 : 참조변수에 타입을 부모 타입으로 선언하는 것)
		Animal a1 = new Tiger(); // Animal 상속 받음 
		Animal a2 = new Eagle();
		Animal a3 = new Shark();
		...
		// 객체 타입 비교연산
		if(a1 instanceof Tiger) {	// a1이 Tiger냐는 조건문
		...
		2) Pet 클래스를 상속받는 Dog, Cat 클래스를 정의하시오.
		 - Dog, Cat 클래스는 makeSount() 메서드를 재정의 하세요.
		 - Dog은 "멍멍" 출력
		 - Cat은 "야옹" 출력
		3) Main 클래스에서 Dog, Cat 객체를 생성하고 정적 메서드 printSound() 를 정의하시오.
		 - main() 메서드에서 printSount() 메서드를 호출하시오.
